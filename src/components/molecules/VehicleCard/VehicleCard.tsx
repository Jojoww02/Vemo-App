  ...
  return vehicleData.status === "approved" ? (
    <div className="flex px-4 md:py-2 gap-6 w-full lg:px-8 bg-white rounded-xl shadow-[0px_3px_7px_5px_#00000040]">
      ...
      <div className="pt-4 pb-3 w-full">
        <h1 className="font-semibold text-sm xs:text-base lg:text-xl md:text-2xl tracking-wide sm:mb-1 md:mb-0">{vehicleData.name}</h1>
        <h3 className="font-semibold text-[.6rem] xs:text-xs md:text-lg lg:text-lg lg:pb-3 text-[#898989]">{vehicleData.licenseNumber}</h3>
          ...
          {window.location.pathname !== REQUEST_MAINTENANCE_VEHICLE_PAGE && (
            <div className="lg:pr-5">
              <Tooltip text="View details vehicle">
                <Link to={VEHICLE_DETAILS_PAGE(vehicleData.id)} className="flex items-center justify-center px-2 py-1 text-center bg-[#F4B400] text-white rounded-lg scale-75 xs:scale-90 lg:scale-110">
