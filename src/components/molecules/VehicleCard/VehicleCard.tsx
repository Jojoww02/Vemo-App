...
export default function VehicleCard({ vehicleData }: Props): JSX.Element {
  ...
  const isRequestMaintenancePage =
  ...
  return vehicleData.status === "pending" ? (
    ...
    <div
      className="flex px-4 py-5 md:py-2 gap-10 w-full lg:px-8 bg-white rounded-xl shadow-[0px_3px_7px_5px_#00000040]"
      onClick={() =>
        isRequestMaintenancePage &&
        navigate(VEHICLE_PARTS_PAGE(vehicleData.vehicleId))
      }
    >
