  ...
  return (
    <div className="flex flex-col w-full items-center">
      <div className="w-full pt- ">
        <h1 className="font-semibold text-3xl xl:text-5xl">Inbox Notification</h1>
        <div className="flex flex-row gap-2 mt-5 mb-10">
          ...
          {tabs.map((tab, index) => (
            <button
              ...
              key={index}
              className={`rounded-lg border-2 px-7 ${tab.id === activeTabEmail ? "bg-[#F4B400] border-[#F4B400] text-white" : "border-[#F4B400] text-[#F4B400]  "}`}
            >
              {tab.name}
            </button>
          ))}
