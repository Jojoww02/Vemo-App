  ...
  return (
    <div className="w-full flex xl:w-[30rem] h-[6.7rem] xl:h-[7rem] xl:px-5 xl:p-2 px-2 p-2 my-2 rounded-[0.50rem] bg-white shadow-[0px_2px_7px_5px_#00000040] cursor-pointer">
      ...
      <div className="relative flex flex-col justify-center px-5 w-full font-semibold text-dark">
        ...
        {isAdmin ? <p className="font-normal text-[0.8rem] sm:text-lg xl:text-lg">Rp. 50.000</p> : <p className="font-normal text-[0.8rem] sm:text-lg xl:text-lg">Kondisi {data.partName} Motor</p>}
        {isCheck && (
          <div className="flex gap-4 items-center absolute top-1 right-8">
            <Dialog>
              ...
              <DialogContent className="w-4/5">
                <DialogHeader className="flex flex-col items-center justify-center">
                  <DialogTitle className="text-2xl font-semibold">mengedit</DialogTitle>
                  <DialogDescription className="text-center">Silahkan kapan terakhir anda services parts ini</DialogDescription>
                </DialogHeader>
            ...
            {!isAdmin && data.condition <= 60 && <Checkbox id={`checkbox-${data.partId}`} onCheckedChange={onCheckboxChange} checked={checked} className="w-6 h-6 md:w-8 md:h-8" />}
          </div>
        )}
