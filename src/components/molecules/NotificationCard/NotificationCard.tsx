...
export default function NotificationCard({ data, deleteMode }: NotificationCardProps) {
  const navigate = useNavigate();
  const formattedDate = typeof data.date === "number" ? new Date(data.date).toLocaleDateString() : data.date.toLocaleDateString();

  return (
    <div className={`w-full ${data.status === 0 ? "bg-[#f4b4004c]" : "bg-[#ECECEC]"} flex flex-col xl:h-[32%] 2xl:h-[21%] h-[22%] mb-5 cursor-pointer relative`}>
      
      <div className="absolute top-2 right-8 border-hidden rounded-full h-5 w-5 cursor-pointer">
      {deleteMode && <Checkbox id={`checkbox-${data.id}`} />}
      </div>
