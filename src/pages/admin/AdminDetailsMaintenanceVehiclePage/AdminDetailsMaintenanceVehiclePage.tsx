...
import { getMaintenancesVehiclesByVehicleIdFn, getVehicleByIdFn, getVehiclePartsConditionFn } from "@/api/services/vehicle";
import { IConditionParts, IMaintenanceVehicle, IMaintenanceVehicleResponse, IUserResponse, IVehicleResponse } from "@/api/types";
...
export default function AdminDetailsMaintenanceVehiclePage() {
  ...
  const { data: maintenanceVehicle } = useQuery<IMaintenanceVehicleResponse, Error> ({
    queryKey: ["vehcileId", vehicleId],
  ...
  console.log("data:", maintenanceVehicle);
      ...
      <div className="w-full flex">
        ...
        <div className="w-1/2 flex flex-col bg-[#F7F8F9] rounded-2xl justify-evenly mt-10 px-5">
          <div className="flex flex-col gap-3 font-semibold text-xl">
            ...
            <p className="font-normal text-lg">"{(maintenanceVehicle?.maintenanceVehicle?.description)}</p>
          </div>
          <div className="flex flex-col gap-3 font-semibold text-xl">
