...
import { getPartByVehicleIdFn, getVehicleByIdFn, getVehiclePartsConditionFn } from "@/api/services/vehicle";
...
export default function VehiclePartsPage(): JSX.Element {
  ...
  const handleSelectAll = () => {
    const allIds = conditionParts!.filter((conditionPart) => conditionPart.condition <= 60).map((conditionPart) => conditionPart.partId);
    ...
    <div className="w-full relative">
      <div className="pt-5 px-10  w-full mb-5">
        <Link to={VEHICLE_DETAILS_PAGE(vehicleId!)} className=" w-[5rem]">
          <img src={"/Icon-arrow.svg"} alt="" className="absolute top-6 left-0 lg:left-5 xl:w-[2rem]  md:w-32 xl:h-[2rem] w-[1.4rem] h-[1.5rem] cursor-pointer" />
        </Link>
      ...
      <div className="relative">
        {isVehicleSuccess && vehicle.status !== "requested" && conditionParts?.some((part) => part.condition < 60) && (
          <button className="absolute -right-1 -top-[3.2rem] md:-top-10 lg:-top-16 items-center text-[9px] xs:text-base md:text-lg text-white bg-primary rounded-lg px-2 py-[7px] xs:py-1" onClick={handleSelectAll}>
        ...
        <div className="flex flex-wrap mx-auto items-center md:w-[70%] lg:w-full lg:justify-evenly pt-5 gap-2">
          {conditionParts?.map((conditionPart) => (
            <PartVehicleCard
              ...
              onCheckboxChange={() => handleCheckboxChange(conditionPart.partId)}
      ...
      <div className="w-full flex place-items-center pt-7 px-14 xs:px-24 sm:px-20 lg:px-48 mb-10">
        <Dialog>
          <DialogTrigger asChild>
            <Button type="button" className="py-6 text-white rounded-md text-base bg-primary xl:text-lg font-medium w-full" disabled={checkedItems.length <= 0 || vehicle?.status === "requested"}>
              {vehicle?.status === "requested" ? <p className="text-xs md:text-base">Perawatan kendaraan sedang diproses</p> : "Request Perawatan"}
            </Button>
          </DialogTrigger>
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent className="w-[95%]">
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="overflow-x-auto">
                          <Table className="text-[.65rem] xs:text-base">
                            <TableCaption className="text-xs xs:text-base text-start">*Perhatian : Harga bisa lebih murah dari estimasi</TableCaption>
                            <TableHeader>
                              <TableRow>
                                <TableHead>Nama Komponen</TableHead>
                                <TableHead>Harga Perawatan</TableHead>
                                <TableHead className="text-right">Harga Jasa</TableHead>
                              </TableRow>
                            </TableHeader>
                            <TableBody>
                              {parts
                                ...
                                .map((part) => (
                                  <TableRow key={part.id}>
                                    ...
                                    <TableCell>{part.maintenancePrice.toLocaleString("id-ID", { style: "currency", currency: "IDR" })}</TableCell>
                                    <TableCell className="text-right">{part.maintenanceServicePrice.toLocaleString("id-ID", { style: "currency", currency: "IDR" })}</TableCell>
                                  </TableRow>
                                ))}
                            </TableBody>
                            <TableFooter>
                              <TableRow>
                                ...
                                <TableCell className="text-right">{totalMaintenanceServicePrice}</TableCell>
                              </TableRow>
                              <TableRow>
                                <TableCell></TableCell>
                                <TableCell></TableCell>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction className="bg-primary hover:bg-primary/80" onClick={methods.handleSubmit(onSubmitHandler)}>
                          Kirim
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
