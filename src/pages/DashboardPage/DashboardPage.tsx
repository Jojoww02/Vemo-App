...
export default function DashboardPage(): JSX.Element {
  ...
  const [showTwoCard, setShowTwoCard] = useState(true);
      ...
      <div className="relative p-4 md:p-7 md:px-10 rounded-2xl shadow-[0px_0px_7px_#00000040] bg-[#898989]">
        <div className="hidden md:block absolute right-2 top-2">
          <Tooltip text="View All Vehicles">
            <IconCircleArrowUpRightFilled size={27} className="text-white hover:scale-125 duration-500" type="button" onClick={handleOnClick} />
          </Tooltip>
        </div>
        <div className={`flex flex-col xs:px-8 md:px-0 md:flex-row items-center gap-4 md:gap-6 ${vehicles?.length === 1 ? "w-1/2" : "w-full"}`}>
          {isSuccess && vehicles.length > 0 ? (
            (vehicles as IVehicleResponse[]).slice(0, showTwoCard ? 2 : vehicles.length).map((vehicle: IVehicleResponse) => <VehicleCard vehicleData={vehicle} key={vehicle.id} />)
          ) : (
