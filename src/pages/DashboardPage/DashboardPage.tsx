    ...
    <React.Fragment>
      <div className="relative p-4 md:p-7 md:px-10 rounded-2xl shadow-[0px_0px_7px_#00000040] bg-[#898989]">
        <div className="w-full flex justify-between">
          ...
          {isSuccess && vehicles && vehicles.length > 2 && (
            <p className="text-white cursor-pointer hover:text-primary transition ease-linear" onClick={() => navigate(VEHICLE_LIST_PAGE)}>
              Lihat semua kendaran
            </p>
          )}
        </div>
        <div className={`flex flex-col xs:px-8 md:px-0 md:flex-row items-center gap-4 md:gap-6`}>
          {isSuccess &&
            (vehicles.length > 0 ? (
              // Jika ada 2 kendaraan
              vehicles.length >= 2 ? (
                (vehicles as IVehicleResponse[]).slice(0, 2).map((vehicle: IVehicleResponse) => <VehicleCard vehicleData={vehicle} key={vehicle.vehicleId} />)
              ) : (
                <>
                  {/* Jika hanya ada 1 kendaraan */}
                  {(vehicles as IVehicleResponse[]).slice(0, 1).map((vehicle: IVehicleResponse) => (
              ...
              <div className="w-full py-4 flex flex-col items-center text-center gap-4">
                ...
                <h1 className="text-white sm:text-xl lg:w-[29rem]">Opps! anda belum mendaftarkan kendaraan anda, daftar dulu yuk!</h1>
              </div>
            ))}
        </div>
      </div>
      <div className="flex flex-col md:flex-row items-center w-full mt-5 gap-5 md:gap-10 mb-10">
        {featuresData.map((feature, index) => (
          <FeatureCard image={feature.image} title={feature.title} key={index} path={feature.path} />
        ))}
      </div>
    </React.Fragment>
  );
