...
export const readNotificationFn = async (notificationId: string): Promise<IGenericResponse> => {
    const response = await privateApi.post<IGenericResponse>(API.READ_NOTIFICATION_SERVICE, { notificationId });
    return response.data;
}
